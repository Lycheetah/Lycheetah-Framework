...
| `[RETRACTED]` | Once held, now disproven, superseded, or scope-corrected. Documented in 28_DEFENSE/FAILURE_MUSEUM.md. | Permanent record. Does not move back up. |
...
4. **Falsifier defined.** A specific outcome is named that would prove the claim false (recorded in `28_DEFENSE/FALSIFICATION_REGISTER.md`).
...
| Trigger | Action |
|---|---|
| A replication attempt with adequate power fails (and the failure is not attributable to methodology error). | Demote to SCAFFOLD; document the failure in 28_DEFENSE/FAILURE_MUSEUM.md; investigate. |
| A formal proof shows the claim is incorrect. | Demote to RETRACTED; full 28_DEFENSE/FAILURE_MUSEUM.md entry; do not delete from history. |
| A scope-revision shows the claim was overstated. | Demote to SCAFFOLD with corrected scope, or RETRACTED if no honest version remains. Original overstated form joins 28_DEFENSE/FAILURE_MUSEUM.md. |
| A measurement reveals the claim was an artifact (data leak, p-hacking, instrument bias, etc.). | RETRACTED. 28_DEFENSE/FAILURE_MUSEUM.md entry. |
...
Audit trail entries are recorded in `28_DEFENSE/CLAIMS.json` (the `provenance` array per claim) and surfaced in change logs. Loss of audit trail is grounds for reversion to the prior status.
...
- **28_DEFENSE/CLAIMS.json** is the live status register. This document defines the rules by which entries change.
- **28_DEFENSE/FALSIFICATION_REGISTER.md** lists the specific falsifiers; this document defines when one is considered "tested."
- **28_DEFENSE/FAILURE_MUSEUM.md** receives RETRACTED claims; this document defines what triggers retraction.
- **29_GOVERNANCE/LIVING_CODEX_PROTOCOL.md** governs document evolution; this document is the per-claim version of the same discipline.
- **28_DEFENSE/NOVEL_CONTRIBUTIONS.md** lists per-claim novelty; this document defines the evidence required to keep the novelty claim ACTIVE.
