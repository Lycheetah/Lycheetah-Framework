...
if __package__ == "lycheetah.applications":
    ...
    from lycheetah import __version__
    from applications.aura_text_checker import AURATextAnalyser, AURATextReport
    from lycheetah.assurance import AssuranceEvent, AssurancePolicy, AssuranceRuntime, Phase
    from core.tri_axial_checker import MetricStatus
    from core.seven_phase import build_cycle
    from core.psi_consensus import build_consensus
    from core.aura_customizer import AURACustomizer, Domain
...
def build_server():
    ...
    if not MCP_AVAILABLE or MCPServer is None:
        raise RuntimeError(
            "[mcp] extra is missing the MCP SDK 2.x server API. Install the compatible "
            "extra with: pip install 'lycheetah-framework[mcp]'"
        )
...
def main(argv: list[str] | None = None) -> None:
    """Run the MCP server on stdio or explicitly selected local HTTP transport."""
    import argparse

    parser = argparse.ArgumentParser(
        prog="lycheetah-guard",
        description="Run the experimental Lycheetah Guard MCP server.",
    )
    transport = parser.add_mutually_exclusive_group()
    transport.add_argument(
        "--http",
        action="store_true",
        help="use local Streamable HTTP instead of stdio",
    )
    transport.add_argument(
        "--stdio",
        action="store_true",
    ...
    if not MCP_AVAILABLE:
        parser.exit(
            1,
            "ERROR: the 'mcp' extra does not provide the required MCP SDK 2.x API.\n"
