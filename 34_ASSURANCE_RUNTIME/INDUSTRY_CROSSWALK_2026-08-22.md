...
|---|---|---|---|
| NIST AI RMF + NIST AI 600-1 GenAI Profile | Govern, map, measure, and manage risk across the AI lifecycle | Versioned policy identity, measured runtime findings, receipts, limitations, review decisions | No organization-wide risk programme, impact assessment, or NIST conformity assessment |
...
| OpenAI Agents SDK | Input/output/tool guardrails, resumable human approvals, trace-driven evals | Matching event phases and three-way disposition; `REVIEW` intended to pause application flow | No claim that a receipt automatically wires itself into every SDK boundary |
