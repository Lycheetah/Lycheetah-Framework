...
**Sources:** SYSTEM_INTEGRATION_GUIDE.md, 30_MAPS/FORMAL_SPINE.md (Act II), 30_MAPS/COHERENCE_REGISTER.md (Act I)
**Depends on:** 30_MAPS/FORMAL_SPINE.md (canonical symbols used throughout)
...
```mermaid
flowchart TB
    AN["ANAMNESIS\nEpistemic ground\n(Why convergence is real)"]
    EL["EARNED LIGHT\nThermodynamic substrate\n(Energy cost of all ops)"]
    LM["LAMAGUE\nNotation stack (Tiers 0–3)\n(Language for everything)"]
    TR["TRIAD\nKernel\n(Ao · Φ↑ · Ψ_op)"]
    AU["AURA\nConstitutional constraints\n(I₁–I₇ always active)"]
    CA["CASCADE\nKnowledge reorganization\n(Π-driven restructuring)"]
    MC["MICROORCIM\nMeasurement\n(μ_drift · τ_phase · σ)"]
    CH["CHRYSOPOEIA\nTransformation calculus\n(Ξ · seven ops · four tiers)"]
    HA["HARMONIA\nResonance calculus\n(C(r) · κ_c · EWM)"]
    IN["INTEGRATIONS\nMaster equation\n(dΨ/dt = k₁Π − k₂Ψ − k₃I + k₄E)"]

    AN -->|"Justifies why\nframeworks converge"| LM
    AN -->|"Grounds ethical\nclaims as discovered"| AU
    EL -->|"Thermodynamic\ncost model"| TR
    EL -->|"Energy floor\nfor operations"| CA
    LM -->|"Notation for\nall operations"| TR
    LM -->|"Formal expression\nof constraints"| AU
    TR -->|"Ao → invariant baseline"| AU
    TR -->|"Φ↑ → reorganization\ngrowth operator"| CA
    TR -->|"Ψ_op → drift\nobservation"| MC
    AU -->|"Constraint set C_set\nfor all Ξ operations"| CH
    AU -->|"I_set constrains\nreorganization method"| CA
    AU -->|"Floors Coh ≥ 0.70\nfor all states"| MC
    CA -->|"Provides Π\nto master equation"| IN
    CA -->|"CASCADE event =\nSep stage of Ξ"| CH
    MC -->|"Drift data\nto master equation"| IN
    MC -->|"Sovereignty score\nfor each agent"| HA
    CH -->|"Transformation depth\n(tier) to master eq."| IN
    HA -->|"Resonance term\nto master equation"| IN
    IN -->|"dΨ/dt: unified\nsystem state"| MC

    style AN fill:#1a1a2e,color:#e0e0e0
    style LM fill:#16213e,color:#e0e0e0
    style EL fill:#0f3460,color:#e0e0e0
    style TR fill:#533483,color:#e0e0e0
    style AU fill:#e94560,color:#ffffff
    style CA fill:#0a3d62,color:#e0e0e0
    style MC fill:#1e8bc3,color:#ffffff
    style CH fill:#c0392b,color:#ffffff
    style HA fill:#d4ac0d,color:#1a1a1a
    style IN fill:#2c3e50,color:#e0e0e0
```
