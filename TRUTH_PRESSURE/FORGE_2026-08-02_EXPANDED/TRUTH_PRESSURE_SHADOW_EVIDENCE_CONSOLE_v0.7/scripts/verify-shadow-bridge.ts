...
import { TRUTH_PRESSURE_SHADOW_STORAGE_KEY } from '../lib/truth-pressure-integration/feature.ts';
import {
  appendShadowRecord,
...
await check('dataset is bounded to configured maximum', async () => {
  ...
  const raw = await store.getItem(TRUTH_PRESSURE_SHADOW_STORAGE_KEY);
