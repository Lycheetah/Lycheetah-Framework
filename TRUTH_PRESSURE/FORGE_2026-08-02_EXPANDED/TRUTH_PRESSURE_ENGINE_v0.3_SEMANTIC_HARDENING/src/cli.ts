...
import { scoreOnionV3 } from './onion-v3.ts';

function read(pathname: string): string { return fs.readFileSync(path.resolve(pathname), 'utf8'); }
function usage(): never {
  console.error(`Truth Pressure Engine v0.3\nUsage:\n  ... analyze-text <text-file>\n  ... score <assessment.json>\n  ... onion <onion.json>\n  ... corpus <corpus.jsonl> [output.jsonl]`);
  process.exit(2);
}
...
try {
  ...
  } else if (command === 'onion') {
    console.log(JSON.stringify(scoreOnionV3(JSON.parse(read(input))), null, 2));
