...
/**
 * React-Native-safe change-detection token.
 * This is deliberately NOT described as a cryptographic hash and must not be used
 * for authentication, privacy, or tamper-proof storage. It only detects stale UI plans.
 */
export function stateToken(value: unknown): string {
...
export function fingerprintLayers<T>(layers: ReviewLayer<T>[]): string {
  return stateToken(layers);
}

function validateLayers<T>(layers: ReviewLayer<T>[]): void {
  if (!Array.isArray(layers) || layers.length !== 9) {
    throw new ValidationError('layers', 'must contain exactly nine onion layers');
  }
...
export function proposeReview<T>(
  ...
  const sourceStateToken = fingerprintLayers(layers);
  ...
  for (const from of candidates) {
    ...
    moves.push({
      from,
      to,
      reason: `${layers[from].name} is weak for an inner layer and should be reviewed at ${layers[to].name}.`,
    });
  }
...
export function applyReviewPlan<T>(layers: ReviewLayer<T>[], plan: ReviewPlan, approved = false): ReviewApplication<T> {
  ...
  const beforeStateToken = fingerprintLayers(before);
  const fail = (error: string): ReviewApplication<T> => ({
    ok: false,
    error,
    approved,
    before,
    after: before.map(x => ({ ...x })),
    beforeStateToken,
    afterStateToken: beforeStateToken,
    plan,
  });
  ...
  if (plan.sourceStateToken !== beforeStateToken) return fail('The layers changed after the plan was proposed; regenerate the plan.');
  ...
  return {
    ...
    after: next,
    beforeStateToken,
    afterStateToken: fingerprintLayers(next),
    plan,
  };
}
