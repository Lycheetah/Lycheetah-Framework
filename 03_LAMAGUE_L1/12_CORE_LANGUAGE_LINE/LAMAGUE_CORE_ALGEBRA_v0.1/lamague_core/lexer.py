...
KEYWORDS = {"let", "invariant", "require", "forbid", "macro", "check", "equivalent"}
...
class Lexer:
    ...
    def tokenize(self) -> tuple[list[Token], list[WarningRecord]]:
        while self.i < len(self.source):
            ...
            if ch == "∮":
                raise LexError(
                    f"reserved symbol '∮' at {line}:{col}; core recursion is '⟲' and '∮' is reserved for integration"
                )
            ...
            for raw in ("<->", "->", "A₀"):
                if self.starts(raw):
                    ...
                    self.warnings.append(WarningRecord(code, message, raw, canonical, line, col))
                    matched = True
                    break
            if matched:
                continue
            ...
            if ch in {"∆", "♾"}:
                ...
                self.warnings.append(WarningRecord(code, message, ch, canonical, line, col))
                continue
    ...
    def _is_ident_continue(ch: str) -> bool:
        return bool(ch) and (ch.isalnum() or ch == "_" or ch in "₀₁₂₃₄₅₆₇₈₉↑")
