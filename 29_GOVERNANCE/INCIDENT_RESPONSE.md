...
**Companion docs:** `29_GOVERNANCE/CONFLICT_OF_INTEREST.md`, `28_DEFENSE/ATTRIBUTION_REQUIREMENTS.md`, `29_GOVERNANCE/LIVING_CODEX_PROTOCOL.md`, `LICENSE`
...
1. **Direct contact, friendly tone.** Email or comparable channel. State the overlap, point to the canonical source, offer the citation. Assume oversight. Provide BibTeX from `28_DEFENSE/CITATIONS.md`. Set expectation of correction in next revision / web edit / upcoming version.
...
1. **Detect.** The defense layer's audit machinery (`28_DEFENSE/ADVERSARIAL_AUDIT_REPORT.md`, `28_DEFENSE/EVIDENCE_LADDER.md`, `28_DEFENSE/FALSIFICATION_REGISTER.md`) is the primary detection mechanism. Run audits on schedule.
2. **Demote or retract.** If a claim has been overstated, demote per `28_DEFENSE/EVIDENCE_LADDER.md`. If a claim is false, retract via `28_DEFENSE/FAILURE_MUSEUM.md` and update `28_DEFENSE/CLAIMS.json`. Audit trail in commit log.
3. **Disclose.** Material internal incidents are disclosed in `29_GOVERNANCE/LIVING_CODEX_PROTOCOL.md` and (if affecting prior publications) corrected via the venue's correction process.
...
- **Not a substitute for license terms.** The license (`LICENSE`, `28_DEFENSE/ATTRIBUTION_REQUIREMENTS.md`) is the operative legal instrument. This protocol describes how the author intends to respond when license terms are violated.
...
**Audit trail:** added in D-1.1 (D2) to address misappropriation risk identified in defense planning. Companion to `29_GOVERNANCE/CONFLICT_OF_INTEREST.md` (B6) — the disclosure surface; this is the response surface.
