...
Forgetting, Truth Pressure, Paradigm Shift, Coherence Preservation, Defeasible Reasoning,
...
When scientific paradigms shift~\citep{kuhn1962structure}---Newtonian mechanics giving
way to general relativity, classical physics to quantum mechanics---the scientific
community does not simply \emph{add} the new theory alongside the old. Nor does it
\emph{delete} the old theory entirely. Instead, it \emph{contextualizes}: Newtonian
mechanics is demoted from ``fundamental law'' to ``excellent approximation valid at low
velocities and weak gravitational fields.'' The old knowledge is preserved but reframed,
while the new knowledge assumes foundational status.
...
replay~\citep{chaudhry2019tiny}, prompt-based approaches~\citep{wang2022learning}---mitigate
forgetting by either protecting old parameters, adding new capacity, rehearsing old
examples, or freezing the backbone entirely. Recent knowledge editing
methods~\citep{meng2022locating,meng2023memit,mitchell2022mend} can surgically modify
individual facts in trained models, but do not address structural reorganization when
an entire paradigm is superseded. All existing approaches assume knowledge is
...
All synthetic experiments were run with
\texttt{cascade\_real\_experiments.py} and historical validations with
\texttt{run\_experiments.py}, both in \texttt{12\_IMPLEMENTATIONS/experiments/}
in the repository. Every number below was computed, not estimated. Random seeds
are fixed and reported for reproducibility.
...
Kuhn's account of scientific revolutions~\citep{kuhn1962structure} is the descriptive
foundation CASCADE formalizes. Kuhn observed that scientific progress proceeds not
through gradual accumulation but through periodic paradigm shifts in which an
anomaly-burdened framework is replaced by an incommensurable new one---and crucially,
that old knowledge is not simply \emph{deleted} but \emph{recontextualized}: Newtonian
mechanics becomes ``an excellent approximation valid at low velocities and weak
gravitational fields.'' G\"{a}rdenfors and the AGM tradition~\citep{alchourron1985logic,gardenfors1988knowledge}
subsequently provided a logical characterization of belief revision: what properties must
a rational revision satisfy? CASCADE bridges these two traditions. Kuhn's observation
that old knowledge is contextualized (not erased) becomes CASCADE's Phase~2 compression;
the anomaly pressure that forces a paradigm shift becomes the truth pressure
inequality~(\ref{eq:trigger}). Where Kuhn is descriptive and AGM is postulational,
CASCADE is algorithmic: a computable protocol that any knowledge system can execute,
with invariant guarantees that hold regardless of the domain.
...
old data. More recently, DER++~\citep{buzzega2020dark} combines knowledge distillation
with replay, and prompt-based methods~\citep{wang2022learning,wang2022dualprompt}
freeze the backbone entirely and learn task-specific prompts. All assume knowledge is
additive; CASCADE reorganizes structure.

\textbf{Belief revision.} The AGM framework~\citep{alchourron1985logic,gardenfors1988knowledge}
defines expansion ($+$), contraction ($-$), and revision ($*$) operators and characterizes
which postulates a rational revision must satisfy. CASCADE's Phase~2 compression is a
form of contraction that preserves the contracted belief in a qualified form---the block
survives the revision with a bounded scope rather than being retracted. The key departure
from AGM is the trigger condition~(\ref{eq:trigger}): AGM revision is operator-driven
(an agent explicitly invokes the revision operator), whereas CASCADE's reorganization is
\emph{evidence-driven}, firing automatically when $\tp$ of the new block exceeds that
of the conflicting foundation by a margin $\Delta\tp$. This makes the revision criterion
computable and auditable without requiring an external agent to initiate it.
...
\textbf{Knowledge editing in language models.}
ROME~\citep{meng2022locating} and MEMIT~\citep{meng2023memit} locate and edit
specific factual associations in transformer weights; MEND~\citep{mitchell2022mend}
learns a hypernetwork to produce targeted weight updates;
KnowledgeEditor~\citep{decao2021editing} uses a learned editor network.
See~\citet{yao2023editing} for a recent survey. These methods modify individual
facts within a trained model---the complementary problem to CASCADE, which
reorganizes the \emph{structural hierarchy} among knowledge claims. A system
that combines CASCADE's reorganization logic with neural knowledge editing
(using $\tp$ to decide \emph{whether} to edit and \emph{how} to restructure
dependent knowledge) is a natural future direction.
...
\textbf{Summary.} Table~\ref{tab:related} positions CASCADE along four axes that
distinguish the approaches surveyed above.

\begin{table}[H]
\centering
\caption{Related approaches compared across four structural properties. A full audit
trail means the complete chain of reasoning---what was promoted, what was contextualized,
and why---is preserved and inspectable after every reorganization event.}
\label{tab:related}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Approach} & \textbf{Audit trail} & \textbf{Old knowledge preserved} & \textbf{Computable trigger} & \textbf{Coherence guarantee} \\
\midrule
AGM Belief Revision        & Axiomatic  & Yes (contraction)     & No (operator-driven)   & Axiomatic \\
Continual Learning (EWC)   & No         & Partial (regularized) & Yes (Fisher importance) & None \\
Neural Knowledge Editing   & No         & No (overwrites)       & Yes (locate-then-edit) & None \\
Knowledge Graphs           & Yes        & Yes (static)          & No trigger criterion   & None \\
Defeasible Logic           & Proof-theoretic & Yes (defeated rules) & Priority ordering   & None \\
\midrule
\textbf{CASCADE (ours)}    & \textbf{Yes} & \textbf{Yes (qualified)} & \textbf{Yes ($\tp$ threshold)} & \textbf{Yes (Thm.~\ref{thm:coherence})} \\
\bottomrule
\end{tabular}
\end{table}
...
CASCADE is one component of the broader Lycheetah Framework~\citep{clark2026lycheetah}, which includes
complementary modules for constitutional constraint checking (AURA), operator
algebras for knowledge state evolution (TRIAD), and a formal notation for
expressing alignment constraints (LAMAGUE). These extensions are documented in
the repository but are outside the scope of this paper, which establishes the
reorganization mechanism in isolation before composing it with other components.
Integration of CASCADE's truth pressure with TRIAD's anchor-observe-correct
convergence cycle is a natural extension; TRIAD's discrete Lyapunov stability
result and the conditions under which the combined system inherits CASCADE's
coherence guarantees are documented in the framework repository.

The most significant open question is not technical but epistemological: does
truth pressure $\tp$ generalize across epistemic traditions, or is it implicitly
a formalization of \emph{Western scientific} paradigm shifts specifically?
Kuhn's original account was grounded in the history of Western natural science.
Whether a knowledge tradition that organizes around whakapapa, kaitiakitanga,
or Confucian precedent produces the same cascade structure under truth pressure---
or requires cultural calibration of the thresholds $\tau_F$ and $\tau_T$---is an
open empirical question with consequences for how CASCADE would be deployed in
governance, education, and cross-cultural AI systems. This is the experiment that
determines whether CASCADE is a mechanism or merely an artifact of its origins.

All code is open-source at \url{https://github.com/Lycheetah/Lycheetah-Framework}.

% ═══════════════════════════════════════════════════════════
\subsection*{Reproducibility}

Synthetic experiments: \texttt{cascade\_real\_experiments.py}.
Historical validations: \texttt{run\_experiments.py} (which calls pluggable domain
modules \texttt{domain\_germ\_theory.py} and \texttt{domain\_quantum\_physics.py}).
All under \texttt{12\_IMPLEMENTATIONS/experiments/}, with the CASCADE engine in
\texttt{12\_IMPLEMENTATIONS/core/cascade\_engine.py}.
Runnable with only NumPy and SciPy.
...
\citet{kuhn1962structure} for the paradigm shift concept that CASCADE formalizes,
to the AGM tradition~\citep{alchourron1985logic,gardenfors1988knowledge} for the
logical foundations of belief revision, and to the continual learning community for
framing the catastrophic forgetting problem. All errors are our own.
...
\bibitem[G\"{a}rdenfors(1988)]{gardenfors1988knowledge}
P.~G\"{a}rdenfors.
...
\bibitem[Wang et~al.(2022a)]{wang2022learning}
...
Z.~Wang, Z.~Zhang, S.~Ebrahimi, R.~Sun, H.~Zhang, C.-Y.~Lee, X.~Ren, G.~Su,
  V.~Perot, J.~Dy, and T.~Pfister.
...
\bibitem[Mitchell et~al.(2022)]{mitchell2022mend}
...
\bibitem[Clark(2026)]{clark2026lycheetah}
M.~C.~J.~Clark.
\newblock The Lycheetah Framework: Nine formal frameworks for {AI} alignment and epistemology (version {C-1.1}).
\newblock Zenodo, 2026.
\newblock \url{https://doi.org/10.5281/zenodo.20020828}.
